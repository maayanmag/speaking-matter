import { getLocale, onLocaleChange, initI18n } from './i18n.js';

const COPY = {
  en: {
    fullscreen: 'Fullscreen', exit: 'Exit fullscreen', language: 'Language',
    fullscreenError: 'Fullscreen is unavailable. The projection will continue in this window.',
    startupError: 'The projection could not start. Please reload the page.',
  },
  he: {
    fullscreen: 'מסך מלא', exit: 'יציאה ממסך מלא', language: 'שפה',
    fullscreenError: 'מסך מלא אינו זמין. ההקרנה תימשך בחלון זה.',
    startupError: 'לא ניתן להתחיל את ההקרנה. יש לטעון את העמוד מחדש.',
  },
};
...
try {
  ...
  document.getElementById('ps-loading').hidden = true;
  localize();
}
